...
import { Plus, Pencil, Trash2, X, BookOpen, Save, AlignLeft, Eye, Search } from 'lucide-react';
import { adminApi } from '../api/adminApi';
import ConfirmationModal from '../components/ConfirmationModal';
import SuccessModal from '../components/SuccessModal';

const EMPTY_DESC = { malayalam: '', english: '', urdu: '' };
const EMPTY = { verse: '', description: { ...EMPTY_DESC } };
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
          <div>
            <label className="flex items-center gap-1.5 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
              <AlignLeft size={14} /> Verse (Arabic) <span className="text-red-500">*</span>
            </label>
            <textarea
              rows={3}
              dir="rtl"
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all w-full max-w-2xl relative z-10 animate-slide-in-right flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
          <div>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Verse</h3>
            <div className="bg-indigo-50/30 p-4 rounded-xl border border-indigo-100/50">
              <p className="text-right text-slate-800 text-2xl font-arabic leading-[2.2] tracking-wide" dir="rtl">{item.verse}</p>
            </div>
          </div>
...
export default function VerseImportancePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-verse-importances'],
    queryFn: () => adminApi.getVerseImportances({ limit: 500 }).then((r) => r.data),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => adminApi.deleteVerseImportance(id),
    onSuccess: () => {
      qc.invalidateQueries(['admin-verse-importances']);
  ...
  const allItems = data?.verse_importances || [];
  const items = allItems.filter((item) => {
    const q = search.trim().toLowerCase();
    return !q || item.verse?.toLowerCase().includes(q) ||
  ...
  return (
    <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col min-h-0 gap-6">
      ...
      <div className="relative max-w-md">
        ...
        <input
          type="text"
          placeholder="Search by verse or description..."
          value={search}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-bold tracking-wider border-b border-slate-100">
                  <tr>
                    <th className="px-6 py-4">Verse</th>
