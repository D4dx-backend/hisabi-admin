        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">{item ? 'Update dhikr category details' : 'Add a new dhikr category'}</p>
            </div>
          </div>
          <button
            onClick={onClose}
...
export default function DhikrCategoriesManagement() {
  ...
  const createMutation = useMutation({
    mutationFn: (d) => adminApi.createDhikrCategory(d),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dhikr-categories']);
      setSuccessState({ isOpen: true, title: 'Success!', message: 'Dhikr category created successfully.' });
      setModal(null);
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => adminApi.updateDhikrCategory(id, data),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dhikr-categories']);
      setSuccessState({ isOpen: true, title: 'Success!', message: 'Dhikr category updated successfully.' });
      setModal(null);
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(['admin-dhikr-categories']);
      setSuccessState({ isOpen: true, title: 'Deleted!', message: 'Dhikr category removed successfully.' });
    ...
    <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col min-h-0 gap-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-24 text-center">
          ...
          <p className="text-slate-500 max-w-sm mx-auto">Add the first dhikr category.</p>
          <button
            onClick={() => setModal('create')}
