...
const EMPTY = { dhikr: '', description: { ...EMPTY_DESC } };
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
          <div>
            <label className="flex items-center gap-1.5 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
              <AlignLeft size={14} /> Dhikr (Arabic) <span className="text-red-500">*</span>
            </label>
            <textarea
              rows={3}
              dir="rtl"
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all w-full max-w-2xl relative z-10 animate-slide-in-right flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
          <div>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Dhikr</h3>
            <div className="bg-indigo-50/30 p-4 rounded-xl border border-indigo-100/50">
              <p className="text-right text-slate-800 text-2xl font-arabic leading-[2.2] tracking-wide" dir="rtl">{item.dhikr}</p>
            </div>
          </div>
...
export default function DhikrImportancePage() {
  ...
  const items = allItems.filter((item) => {
    ...
    return !q || item.dhikr?.toLowerCase().includes(q) ||
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="relative max-w-md">
        ...
        <input
          type="text"
          placeholder="Search by dhikr or description..."
          value={search}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-bold tracking-wider border-b border-slate-100">
                  <tr>
                    <th className="px-6 py-4">Dhikr</th>
                    ...
                    <tr key={item.id} onClick={() => setViewItem(item)} className="hover:bg-slate-50/80 transition-colors group cursor-pointer">
                      <td className="px-6 py-4 max-w-xs">
                        <p className="font-arabic text-slate-700 text-lg leading-loose text-right truncate" dir="rtl">
                          {item.dhikr?.length > 80 ? item.dhikr.substring(0, 80) + '...' : item.dhikr}
                        </p>
                      </td>
