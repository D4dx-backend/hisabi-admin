...
import { Plus, Pencil, Trash2, X, BookHeart, Save, AlignLeft, Eye, Search } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all w-full max-w-2xl relative z-10 animate-slide-in-right flex flex-col max-h-[90vh]">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 font-display">{item ? 'Edit Dua Importance' : 'Add Dua Importance'}</h2>
              <p className="text-xs text-slate-500 font-medium">{item ? 'Update dua importance content' : 'Add a new dua importance entry'}</p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all w-full max-w-2xl relative z-10 animate-slide-in-right flex flex-col max-h-[90vh]">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 font-display">Dua Importance Details</h2>
...
export default function DuaImportancePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-dua-importances'],
    queryFn: () => adminApi.getDuaImportances({ limit: 500 }).then((r) => r.data),
  });

  const createMutation = useMutation({
    mutationFn: (d) => adminApi.createDuaImportance(d),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dua-importances']);
      setSuccessState({ isOpen: true, title: 'Success!', message: 'Dua importance created successfully.' });
      setModal(null);
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => adminApi.updateDuaImportance(id, data),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dua-importances']);
      setSuccessState({ isOpen: true, title: 'Success!', message: 'Dua importance updated successfully.' });
      setModal(null);
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => adminApi.deleteDuaImportance(id),
    onSuccess: () => {
      qc.invalidateQueries(['admin-dua-importances']);
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-2 ml-1">Manage the {allItems.length} dua importance entries.</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-24 text-center">
          ...
          <p className="text-slate-500 max-w-sm mx-auto">Add the first dua importance entry.</p>
          <button
            onClick={() => setModal('create')}
