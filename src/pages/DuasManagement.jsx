...
import { Plus, Pencil, Trash2, X, Activity, Save, Type, Languages, ChevronLeft, ChevronRight, BookOpen } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden scale-in-center">
        <div className="flex items-center justify-between px-8 py-6 bg-slate-50 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">{item ? 'Edit Dua' : 'Create Dua'}</h2>
              <p className="text-sm font-medium text-slate-500 mt-0.5">{item ? 'Modify existing catalog entry' : 'Add new dua to library'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="col-span-1 bg-slate-50 p-5 rounded-2xl border border-slate-100">
              <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2">
                <Type size={16} className="text-slate-400" /> Title <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                ...
                onChange={set('title')}
              />
              <p className="text-xs text-slate-500 font-medium mt-2 ml-1">Title to identify this dua inside the application.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <div key={key}>
                  <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2">
                    <span className="text-slate-400">{icon}</span> {label} {key === 'arabic_text' && <span className="text-red-500">*</span>}
                  </label>
                  <textarea
                    rows={3}
...
export default function DuasManagement() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-duas', page],
    queryFn: () => adminApi.getDuas({ page, limit }).then((r) => r.data),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSuccessState({ isOpen: true, title: 'Created', message: 'Dua catalog item has been created.' });
      setModal(null);
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(['admin-duas']);
      setSuccessState({ isOpen: true, title: 'Updated', message: 'Dua catalog item has been updated.' });
      setModal(null);
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(['admin-duas']);
      setSuccessState({ isOpen: true, title: 'Deleted', message: 'Dua catalog item has been permanently removed.' });
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage global library of duas that users can access</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50/80 text-slate-500 text-[10px] uppercase font-bold tracking-wider border-b border-slate-100">
              <tr>
                <th className="px-6 py-4">Dua Title / Key</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="font-bold text-slate-600 text-base mb-1">No Duas found</p>
                ...
                <tr key={dua.id} onClick={() => setViewItem(dua)} className="hover:bg-slate-50/80 transition-colors group cursor-pointer">
                  <td className="px-6 py-4">
                    ...
                    <div className="font-bold text-slate-800">{dua.title}</div>
                  </td>
                  <td className="px-6 py-4 text-right" dir="rtl">
                    <span className="font-bold text-lg text-emerald-800 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100/50 block w-max ml-auto max-w-[300px] truncate" title={dua.arabic_text}>
                      {dua.arabic_text || '—'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      {dua.english && <span className="bg-blue-50 text-blue-700 border border-blue-100 px-2 py-1 rounded-lg text-xs font-bold">EN</span>}
                      {dua.malayalam && <span className="bg-indigo-50 text-indigo-700 border border-indigo-100 px-2 py-1 rounded-lg text-xs font-bold">ML</span>}
                      {dua.urdu && <span className="bg-emerald-50 text-emerald-700 border border-emerald-100 px-2 py-1 rounded-lg text-xs font-bold">UR</span>}
                      {!dua.english && !dua.malayalam && !dua.urdu && <span className="text-slate-400 font-medium italic">No translations</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => { e.stopPropagation(); setModal(dua); }} className="p-2 rounded-xl text-slate-400 bg-white border border-slate-100 hover:bg-indigo-50 hover:text-indigo-600 transition-colors shadow-sm" title="Edit Item">
                        <Pencil size={16} />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); setItemToDelete(dua); }} className="p-2 rounded-xl text-slate-400 bg-white border border-slate-100 hover:bg-red-50 hover:text-red-600 transition-colors shadow-sm" title="Delete Item">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
